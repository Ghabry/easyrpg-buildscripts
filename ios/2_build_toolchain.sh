...
function set_build_flags_sim {
	CCOMPILER=$(xcrun --sdk iphonesimulator --find clang)
	CXXCOMPILER=$(xcrun --sdk iphonesimulator --find clang++)
	...
	export PLATFORM_PREFIX=$WORKSPACE/iphonesimulator

	set_build_flags
}

function set_build_flags_ios {
	CCOMPILER=$(xcrun --sdk iphoneos --find clang)
	CXXCOMPILER=$(xcrun --sdk iphoneos --find clang++)
	SDKPATH=$(xcrun --sdk iphoneos --show-sdk-path)

	export CC="$CCOMPILER -isysroot $SDKPATH -arch armv7 -arch armv7s -arch arm64"
	export CPP="$CCOMPILER -isysroot $SDKPATH -arch armv7 -E"
	export CXXCPP="$CXXCOMPILER -isysroot $SDKPATH -arch armv7 -E"
	export CXX="$CXXCOMPILER -isysroot $SDKPATH -arch armv7 -arch armv7s -arch arm64"

	export TARGET_HOST=arm-apple-darwin
	export PLATFORM_PREFIX=$WORKSPACE/iphoneos
...
function install_lib_sdl {
	...
	cd SDL2-2.0.5
	# Lie about the triplet because otherwise the iphoneos .m files are not pulled in
	CFLAGS="-fobjc-arc $CFLAGS" ./configure --host=arm-apple-darwin --prefix=$PLATFORM_PREFIX \
		--disable-shared --enable-static
